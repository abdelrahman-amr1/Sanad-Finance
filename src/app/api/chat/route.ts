...
export async function POST(req: NextRequest) {
  try {
    ...
    const { action, query, committee, draftParams, apiKey } = body;

    // Call the direct server-side implementation by bypassing the client-side check
    if (action === 'askTaxQuestion') {
      const result = await geminiService.askTaxQuestion(query, true, apiKey);
      return NextResponse.json(result);
    } else if (action === 'analyzeCommittee') {
      const result = await geminiService.analyzeCommittee(committee, true, apiKey);
      return NextResponse.json(result);
